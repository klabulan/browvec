...
class S {
  ...
  setupWorkerListeners() {
    this.worker.onmessage = (e) => {
      ...
      this.log("error", "Worker error:", e.message), this.rejectAllPending(new l("Worker error: " + e.message, "WORKER_ERROR"));
    }, this.worker.onmessageerror = (e) => {
      this.log("error", "Worker message error:", e), this.rejectAllPending(new l("Worker message error", "MESSAGE_ERROR"));
    };
  }
  ...
  call(e, t) {
    return new Promise((r, i) => {
      if (this.pendingCalls.size >= this.config.maxConcurrentOperations) {
        this.log("error", `Rate limit exceeded for ${e}: ${this.pendingCalls.size}/${this.config.maxConcurrentOperations}`), i(new l(
      ...
      const s = this.generateCallId(), n = Date.now(), o = setTimeout(() => {
        this.log("error", `Operation timeout for ${e} after ${this.config.operationTimeout}ms`), this.pendingCalls.delete(s), this.performanceMetrics.timeouts++, i(new l(
      ...
      try {
        ...
        this.log("error", `Failed to send RPC message for ${e}:`, p), this.pendingCalls.delete(s), clearTimeout(o), i(new l(
  ...
  async close() {
    ...
    return this.rejectAllPending(new l("Worker closed", "WORKER_CLOSED")), e;
  }
  async exec(e) {
  ...
  terminate() {
    this.rejectAllPending(new l("Worker terminated", "TERMINATED")), this.worker.terminate();
  }
}
class q {
  constructor(e = {}) {
  ...
  async handleMessage(e) {
    ...
    try {
      ...
      if (!r)
        throw new l(`Unknown method: ${e.method}`, "UNKNOWN_METHOD");
      ...
      this.log("error", `Method ${e.method} failed:`, r), t = {
        ...
        error: {
          ...
          code: r instanceof l ? r.code : "UNKNOWN_ERROR",
...
class d extends c {
  constructor(e, t, r = "PROVIDER_ERROR", i, s) {
    ...
    ), this.name = "ProviderError", this.providerName = t, this.modelVersion = i, Object.setPrototypeOf(this, d.prototype);
  }
}
class v extends d {
  constructor(e, t, r, i) {
...
class b extends c {
  constructor(e, t, r, i) {
    ...
    ), this.name = "TimeoutError", this.timeoutMs = t, this.operation = r, Object.setPrototypeOf(this, b.prototype);
  }
}
class k extends c {
  constructor(e, t, r, i) {
...
class y extends T {
   ...
   */
  shouldRetry(e) {
    return e instanceof g || e instanceof c && e.category === "configuration" ? !1 : e instanceof u ? e.recoveryInfo?.canRetry ?? !1 : e instanceof w || e instanceof b;
  }
  /**
  ...
  calculateRetryDelay(e, t) {
    ...
    t instanceof u && (r = t.recoveryInfo?.retryAfter || 2e3), t instanceof b && (r = Math.min(t.timeoutMs * 0.5, 5e3));
  ...
  wrapError(e) {
    return e instanceof c ? e : e.message.includes("401") || e.message.includes("Unauthorized") ? new g(
      ...
      { provider: this.name, originalError: e.message }
    ) : e.message.includes("timeout") || e.name === "TimeoutError" ? new b(
      "Request timeout",
...
class C extends T {
  ...
  async generateEmbedding(e) {
    ...
    try {
      ...
      const i = Date.now() - t;
      throw this.updateMetrics(i, 1, !0), r instanceof Error ? r : new d(
  ...
  async generateBatch(e) {
    ...
    try {
      ...
      const i = Date.now() - t;
      throw this.updateMetrics(i, e.length, !0), r instanceof Error ? r : new d(
  ...
  async sendMessage(e, t, r) {
    ...
    return new Promise((i, s) => {
      const n = `${e}_${++this.messageCounter}_${Date.now()}`, o = r || this.config.operationTimeout || 1e4, h = setTimeout(() => {
        this.pendingRequests.delete(n), s(new b(
...
class L {
  ...
  async getAvailableModels() {
    const e = [];
    return R() && e.push(E()), e.push(...I.getAvailableModels()), e;
  }
  /**
  ...
  getProviderRecommendations(e) {
    ...
    if ((n === "local" || n === "any") && R() && r === 384 && t.push({
      provider: "transformers",
      dimensions: 384,
...
//# sourceMappingURL=ProviderFactory-B-tX-hnV.mjs.map
